<?php
session_start();

require_once(__DIR__  . "/connexionDB.php");
require_once(__DIR__ . "/isconnected.php");

$getData = $_GET["id"];


if (!isset($getData) || !is_numeric($getData)) {
    echo "aucun élément ne correspond";
    exit;
}

$request = "SELECT * FROM recipes WHERE recipe_id = :recipe_id";

$recipesStatement = $mysqlClient->prepare($request);

$recipesStatement->execute([
    ":recipe_id" => (int)$getData, // le (int) est là pour le convertir en entier vu qu'il est une string dans le tableau associatif
]);

$recipe = $recipesStatement->fetch(PDO::FETCH_ASSOC);
/**
 * PDO::FETCH_ASSOC est une constante de classe qui affichera 
 * les éléments sous la forme d'un tableau associatif.
 * 
 * Les constantes de classe sont définies avec le mot-clé const à l'intérieur
 * d'une classe et sont appelées en utilisant la syntaxe :
 * NomDeLaClasse::NOM_DE_LA_CONSTANTE
 * 
 * Une constante de classe est une valeur qui est définie dans une classe
 *  et qui ne change pas.
 *  Elle est accessible depuis n'importe quelle instance de la classe 
 * ou même directement depuis la classe elle-même
 * 
 */
?>

<!DOCTYPE html>
<html>

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Site de Recettes - Edition de recette</title>
    <link
        href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css"
        rel="stylesheet">
</head>

<body class="d-flex flex-column min-vh-100">
    <div class="container">

        <?php require_once(__DIR__ . '/header.php'); ?>
        <h1>Mettre à jour <?php echo ($recipe['title']); ?></h1>
        <form action="recipes_post_update.php" method="POST">
            <div class="mb-3 visually-hidden">
                <label for="id" class="form-label">Identifiant de la recette</label>
                <input type="hidden" class="form-control" id="id" name="id" value="<?php echo ($getData); ?>">
            </div>
            <div class="mb-3">
                <label for="title" class="form-label">Titre de la recette</label>
                <input type="text" class="form-control" id="title" name="title" aria-describedby="title-help" value="<?php echo ($recipe['title']); ?>">
                <div id="title-help" class="form-text">Choisissez un titre percutant !</div>
            </div>
            <div class="mb-3">
                <label for="recipe" class="form-label">Description de la recette</label>
                <textarea class="form-control" placeholder="Seulement du contenu vous appartenant ou libre de droits." id="recipe" name="recipe"><?php echo $recipe['recipe']; ?></textarea>
            </div>
            <button type="submit" class="btn btn-primary">Envoyer</button>
        </form>
        <br />
    </div>

    <?php require_once(__DIR__ . '/footer.php'); ?>
</body>

</html>